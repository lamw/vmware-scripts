...
# Description: Using cURL to inteact with the new NSX-T Policy API in VMC to list USER NAT Rules in CGW
...
NSXT_NAT_USER_URL="${NSXT_PROXY_URL}/policy/api/v1/infra/tier-1s/cgw/nat/USER/nat-rules"

RESULTS=$(curl -s -X GET -H "Content-Type: application/json" -H "csp-auth-token: ${CSP_ACCESS_TOKEN}" ${NSXT_NAT_USER_URL})
echo ${RESULTS} | jq